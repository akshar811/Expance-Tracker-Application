const Expense = require("../models/Expense_Schema");
const fs = require("fs");
  ...
  try {
    fs.createReadStream(req.file.path)
      ...
      .on("end", async () => {
        ...
        res
          .status(200)
          .json({ success: true, message: "Bulk expenses added successfully" });
      });
  } catch (err) {
...
const getExpenses = async (req, res) => {
  ...
  try {
    ...
    const expenses = await cacheData(
      `expenses:${JSON.stringify(filters)}:${page}`,
      async () => {
...
const deleteExpenses = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Expenses deleted successfully" });
  } catch (err) {
    res.status(500).json({ success: false, error: err.message });
  }
};
